...
from app.routes.auth import get_current_user as _get_current_user, create_access_token

# Local helpers to align with names referenced in this module
...
async def get_user_profile(current_user: Dict = Depends(_get_current_user)):
	"""Get current user profile information"""
	try:
		user = await db.users.find_one({"user_id": current_user["id"]})
		if not user:
...
async def change_password(request: PasswordChangeRequest, current_user: Dict = Depends(_get_current_user)):
...
async def change_email(request: EmailChangeRequest, current_user: Dict = Depends(_get_current_user)):
	"""Change user email"""
	try:
		user = await db.users.find_one({"user_id": current_user["id"]})
		if not user:
		...
		if existing_user and existing_user["user_id"] != current_user["id"]:
...
async def change_username(request: UsernameChangeRequest, current_user: Dict = Depends(_get_current_user)):
	"""Change username"""
	try:
		# Update username
		await db.users.update_one(
...
async def delete_user_history(request: DeleteHistoryRequest, current_user: Dict = Depends(_get_current_user)):
	...
	try:
		if request.assessment_ids:
			...
			result = await db.user_assessments.delete_many({"user_id": current_user["id"]})
...
async def delete_user_account(current_user: Dict = Depends(_get_current_user)):
	"""Delete user account and all associated data"""
	try:
		# Delete all user assessments
		await db.user_assessments.delete_many({"user_id": current_user["id"]})
		
		# Delete user record
		await db.users.delete_one({"user_id": current_user["id"]})
...
async def get_assessment_history(current_user: Dict = Depends(_get_current_user)):
	"""Get user's assessment history with IDs for deletion"""
	try:
		assessments = await db.user_assessments.find({"user_id": current_user["id"]}).to_list(length=None)
...
async def delete_single_assessment(assessment_id: str, current_user: Dict = Depends(_get_current_user)):
